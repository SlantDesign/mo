...
class ResonateShapeGenerator {
    ...
    func createRandomShape(_ center: Point) -> (Gradient, NSData) {
    ...
    func generateShapeFrom(_ center: Point, pathIndex: Int, angle: Double) -> (Gradient, NSData) {
        ...
        data.append(&p, length: MemoryLayout<Point>.size)
        data.append(&pi, length: MemoryLayout<Int>.size)
        data.append(&a, length: MemoryLayout<Double>.size)

        return (gradient, data)
    }

    func rebuildShape(_ data: NSData) -> Gradient {
        var index = 0
